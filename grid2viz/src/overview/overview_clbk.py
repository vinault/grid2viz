...
def register_callbacks_overview(app):
    ...
    @app.callback(Output("nb_attacks_card", "children"), [Input("scenario", "data")])
    def update_card_attack(scenario):
        """Display the number of hazard of the best agent when page is loaded."""
        best_agent_ep = make_episode(best_agents[scenario]["agent"], scenario)
        nb_attacks = len(best_agent_ep.attacks_data_table.loc[best_agent_ep.attacks_data_table["attack"], "id_lines"].unique())
        return nb_attacks
    #
    # @app.callback(
